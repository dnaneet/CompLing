...
if selection == "Meta-discourse analysis":
  ...
  with st.expander("See explanation"):
    st.write("Metadiscourse is how we write or say things.  When we use words like 'but', 'and', 'maybe', 'definitely', 'I/we/you...', we are using metadiscourse  .  This is the discourse (written or spoken communication) about the _discourse_!  What is _very_ interesting is that each type of writing (called "genre of writing") has its own unique metadiscursive signature.  For _example_, abstracts to scientific articles have a different metadiscursive signature than newspaper articles.  This is because the authors are saying things differently or taking a stance that is unique or particular to that type of writing or speech.  There are two major classes of metadisourse markers, viz., interactional metadiscourse and interactive metadiscourse.  International metadiscourse are those markers employed by the author or the speaker to interact with the reader or the audience and attempt to put them in his/her shoes.  Interactive metadiscourse are those markers employed by the author or speaker to help the reader interact with the content.  Some examples of interactional metadiscourse are: I, me, we, you, us, amazingly, hopefully, curiously, doubt, likely, maybe, etc.  Some examples of interactive metadiscourse are: firstly, secondly, next, then, according [to], but, since, claim [to], etc.  There are about 200 metadiscourse markers in the English language.  Although these account for less than 10% of a native English speaker's vocabulary, they are important in relaying content or data to an audience.")
